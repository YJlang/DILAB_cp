...
> ⚠️ **이 문서는 운영 *원본* 기준**(현행 Cloudflare+Modal+Supabase 아키텍처). 현재 작업 트리는 Oracle 전환용 분리된 복제본 — 배포 절차는 원본 소유. 복제본의 Oracle 전환 로드맵 → [ORACLE_MIGRATION_PLAN.md](ORACLE_MIGRATION_PLAN.md). (AS_OF 2026-06-23)
>
